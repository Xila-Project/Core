 ...
 * @return return 0 if success
 */
int os_thread_detach(korp_tid Thread) { return xila_thread_detach(Thread); }
 ...
 */
void os_thread_exit(void *retval) { xila_thread_exit(); }
 ...
 */
void os_begin_blocking_op() { xila_thread_begin_blocking_operation(); }

/**
 * Stop accepting os_wakeup_blocking_op requests for the calling thread.
 */
void os_end_blocking_op() { xila_thread_end_blocking_operation(); }
 ...
 * system call.
 */
int os_wakeup_blocking_op(korp_tid tid) { return xila_thread_wake_up(tid); }
 ...
 */
os_file_handle os_get_invalid_handle() { return LONG_MAX; }

os_raw_file_handle os_invalid_raw_handle() { return LONG_MAX; }
...
__wasi_errno_t os_fstat(os_file_handle handle, struct __wasi_filestat_t *buf) {
  ...
  XilaFileSystemResult file_system_result =
      xila_file_system_get_statistics(handle, &file_system_statistics);
...
__wasi_errno_t os_fstatat(os_file_handle handle, const char *path,
  ...
  XilaFileSystemResult file_system_result =
      xila_file_system_get_statistics_from_path(path, &file_system_statistics,
                                                follow_symlink);
...
__wasi_errno_t os_file_get_fdflags(os_file_handle handle,
  ...
  if (result == 0) {
    *flags = into_wasi_fdflags(status);
  }
  ...
  if (oflags & __WASI_O_DIRECTORY) {
    ...
    if (path[0] != '/') {
      ...
      return into_wasi_error(
          xila_file_system_open(new_path, mode, open, status, out));
    }

    return into_wasi_error(
        xila_file_system_open(path, mode, open, status, out));
  }
}
...
os_file_get_access_mode(os_file_handle handle,
  ...
  XilaFileSystemResult file_system_result =
      xila_file_system_get_access_mode(handle, &mode);
 ...
 */
__wasi_errno_t os_seekdir(os_dir_stream dir_stream,
                          __wasi_dircookie_t position) {
  XilaFileSystemResult Result =
      xila_file_system_directory_set_position(dir_stream, position);
...
__wasi_errno_t os_readdir(os_dir_stream dir_stream, __wasi_dirent_t *entry,
  ...
  XilaFileSystemResult file_system_result =
      xila_file_system_read_directory(dir_stream, d_name, &type, &size, &inode);
 ...
 */
os_dir_stream os_get_invalid_dir_stream() { return 0xFFFFFFFFFFFFFFFF; }
