...
pub struct ControlDirection(u8);
...
impl ControlCommand {
    ...
    pub const fn new<A>(direction: ControlDirection, kind: u8, number: u8) -> Self {
    ...
    pub const fn get_direction(&self) -> ControlDirection {
        let direction = (self.0 >> Self::DIRECTION_SHIFT) & ((1 << Self::DIRECTION_SIZE) - 1);
        ControlDirection(direction as u8)
    }
